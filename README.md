...
- **可见首图对照页**：`share.html`（Vercel 地址为 `https://lyc-wsy-wedding.click/share`）把同一张 400×400 图片正常显示为首图，没有脚本、离屏定位、隐藏或自动跳转；按钮进入完整请柬。用于区分离屏首图问题与微信不采用自动取图的问题。需在微信里打开此页、等照片出现后从右上角菜单重新分享，仍以实际卡片为验收；当前尚未确认该方案在用户微信中成功。
